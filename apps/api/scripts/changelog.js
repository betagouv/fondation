...
function logCommits(commandRange) {
  /** @type string */
  const list = execSync(
    `git log --pretty='%h %s' ${commandRange[0]}..${commandRange[1]}`,
    { encoding: 'utf-8' },
  );
...
function decorateCommit(commit) {
  ...
  const issueRe = new RegExp(
    `(?<type>fix|feat|refactor|docs)\\((?<issueId>${TRGM}-\\d+)\\): `,
  );
  ...
  outputCommit = outputCommit.replace(
    repoIssueRe,
    (_match, magic, issueId, offset, str) => {
...
function main() {
  ...
  assert.ok(
    typeof source === 'string' && /^\w+$/.test(source),
    'expected a commit',
  );

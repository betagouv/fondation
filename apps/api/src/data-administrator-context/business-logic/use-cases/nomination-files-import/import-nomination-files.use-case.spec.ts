...
import { FakeDomainEventRepository } from 'src/shared-kernel/adapters/secondary/repositories/fakeDomainEventRepository';
...
describe('Import Nomination Files Use Case', () => {
  ...
  it('parses a line with possible empty values unfilled and one rule pre-validated at true', async () => {
    ...
    await new ImportNominationFilesUseCase(
      ...
      transactionPerformer,
      domainEventRepository,
    ).execute(fileToImportWithOptionalsOneRuleNotPrevalidated);

    expect(nominationFileRepository.nominationFiles).toEqual<
      FakeNominationFileRepository['nominationFiles']
    >({
      1: getLucienPierreModel(nominationFileUuid!, 1),
    });
    expect(domainEventRepository.events).toEqual<
      NominationFilesImportedEvent[]
    >([
  ...
  it('saves two lines', async () => {
    ...
    await new ImportNominationFilesUseCase(
      ...
      transactionPerformer,
      domainEventRepository,
    ).execute(fileToImportWithMultipleLines);

    expect(nominationFileRepository.nominationFiles).toEqual<
      FakeNominationFileRepository['nominationFiles']
    >({
      1: getMarcelDupontModel(firstUuid!, 1),
      2: getLucienPierreModel(secondUuid!, 2),
    });
    expect(domainEventRepository.events).toEqual<
      NominationFilesImportedEvent[]
    >([
      new NominationFilesImportedEvent(
        nominationFilesImportedEventId,
        {
          [getMarcelDupontRead(1).rowNumber]: getMarcelDupontRead(1),
          [getLucienPierreRead(2).rowNumber]: getLucienPierreRead(2),
        },
        dateTimeProvider.currentDate,
      ),
    ]);

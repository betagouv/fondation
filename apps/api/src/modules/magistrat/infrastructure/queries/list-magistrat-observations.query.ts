  ...
  async handle(query: {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const sessionIds = Array.from(
        new Set(observations.map(({ nominationFile }) => nominationFile.session.id)),
      );
      ...
      const items = observations.map((observation) => ({
        ...
        nominationFile: this.toFileDto(observation.nominationFile, publishedVersionIds, reportedSessionIds),
      }));
...
const NominationFileSchema = z.object({
  id: z.string(),
  name: z.string(),
...
const MagistratObservationSchema = z.object({
  ...
  nominationFile: NominationFileSchema,
});

...
export const drizzleExcludedJurisdictions =
  dataAdministrationContextSchema.table(
    ...
    (t) => ({
      primaryKey: primaryKey({ columns: [t.userId, t.jurisdictionId] }),
    }),
  );

export const drizzleExcludedJurisdictionRelations = relations(
  drizzleExcludedJurisdictions,
  ({ one }) => ({
    user: one(users, {
      fields: [drizzleExcludedJurisdictions.userId],
      references: [users.id],
    }),
    jurisdiction: one(drizzleJurisdiction, {
      fields: [drizzleExcludedJurisdictions.jurisdictionId],
      references: [drizzleJurisdiction.codejur],
    }),
  }),
);

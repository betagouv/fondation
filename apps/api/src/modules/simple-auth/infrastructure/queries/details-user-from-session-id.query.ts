...
export class DetailsUserFromSessionIdQuery {
  ...
  async handle(query: { sessionId: string }): Promise<{ id: string; role: string } | null> {
    return Sentry.startSpan({ name: 'fr.csm.fondation:queries:detailsUserFromSessionIdQuery' }, () =>
      this._handle(query),
    );
  }
}

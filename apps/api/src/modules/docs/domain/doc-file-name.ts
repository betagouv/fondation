...
import { Magistrat, TypeDeSaisine, TypeDeSaisineLabels } from 'shared-models';
...
export function docFileName(file: {
  ...
  formation: 'SIEGE' | 'PARQUET' | Magistrat.Formation | null;
  date: Date;
}): string {
  const docType = file.type === 'AGENDA' ? 'ODJ' : file.type === 'OFFICIAL_REPORT' ? 'PV' : 'NDR';

  const date = format(file.date, 'dd-MM-yyyy');
  const formationLabel = file.formation
    ? file.formation === Magistrat.Formation.SIEGE
      ? 'Siège'
      : 'Parquet'
    : null;

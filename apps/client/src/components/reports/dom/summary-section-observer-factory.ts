...
const notifyMostVisibleSectionFactory = (notify: Notify) => (entries: IntersectionObserverEntry[]) =>
  entries.forEach((entry) => {
    if (entry.isIntersecting && !isScrollingToSummarySection) notify(entry.target.id);
  });

export const summarySectionsObserverFactory = (sectionIds: string[], notify: Notify) => {

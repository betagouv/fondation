...
import { MemberReports, type MemberReport } from './member-reports';

const listed = (nominationFileId: string, report: MemberReport) => ({
  name: 'KOFFI Aminata',
  nominationFileId,
  number: 1,
  report,
...
describe('MemberReports', () => {
  it('finds the report of a nomination file', () => {
    const model = MemberReports.fromReports([
      listed('file-1', { id: 'report-1', state: 'IN_PROGRESS' }),
      listed('file-2', { id: 'report-2', state: 'NEW' }),
    ]);
  ...
  it('has no report for a file the member does not report on', () => {
    const model = MemberReports.fromReports([listed('file-1', { id: 'report-1', state: 'IN_PROGRESS' })]);

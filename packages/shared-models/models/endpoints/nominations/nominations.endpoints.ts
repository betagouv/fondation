...
export interface NominationsContextTransparenceRestContract extends RestContract {
...
export const sessionSnapshotParamsSchema = z.object({
  sessionId: z.uuid(),
}) satisfies ZodParamsDto<NominationsContextSessionsRestContract, 'sessionSnapshot'>;

export interface TransparenceSnapshotQueryParamsDto extends Record<string, string | number> {
  nom: string;
  formation: Magistrat.Formation;
...
export const transparenceSnapshotQueryParamsSchema = z.object({
  ...
  formation: z.enum(Magistrat.Formation, {
    error: () => 'La formation est requise.',
  }),
  year: z.coerce
    .number()
    .int()
